...
const DA_SCARTARE = new Set([
  ...
  // Aggettivi e quantificatori che nelle descrizioni fanno la frase e nei
  // titoli dei prodotti pescano tutt'altro. "Poche linee pulite" di Minimal
  // faceva comparire "BURGUNDY SMALL VIDE POCHE" — un vuotatasche — fra i
  // capi consigliati, perché "poche" è dentro "poche".
  "poche", "pochi", "poca", "pulite", "pulito", "pulita", "puliti",
  "nessun", "nessuna", "nessuno", "dettaglio", "dettagli", "inutile",
  "inutili", "pieno", "piena", "pieni", "piene", "linee", "forme",
  "grande", "grandi", "piccolo", "piccola", "piccoli", "piccole",
  "morbido", "morbida", "leggero", "leggera", "esiste", "esistono",
...
/**
 * I capi di uno stile, scelti fra righe già scaricate.
 *
 * Le stesse parole, dentro il database, sono la cosa più cara che ci sia:
 * ognuna viene confrontata con cinque campi di ogni riga — uno è la
 * descrizione, seicento caratteri — su settantottomila righe, e la ricerca
 * sfora i tre secondi che il database concede. Qui costano un millisecondo,
 * perché le righe sono già in mano.
 *
 * E si usano tutte. Al database se ne mandavano al massimo cinque per non
 * ammazzarlo; qui la lista intera non pesa niente, e uno stile riconosciuto
 * da otto parole è riconosciuto meglio che da cinque.
 */
export function capiDelloStile(capi, nome) {
  if (!nome) return capi || [];
  const parole = paroleDelloStile(nome).map((p) => p.toLowerCase());
  if (!parole.length) return capi || [];
  return (capi || []).filter((c) => {
    const testo = `${c.titolo || ""} ${c.categoria || ""} ${c.marca || ""} ${c.colore_nome || ""} ${c.descrizione || ""}`.toLowerCase();
    return parole.some((p) => testo.includes(p));
  });
}

/**
 * Le parole di TUTTA la famiglia a cui lo stile appartiene.
 *
 * Serve quando di uno stile, nei colori di una persona e dentro il suo
 * budget, in catalogo non c'è niente. "Blokecore" può non avere nulla, ma
 * "Urbani e street" — skate, streetwear, cargo — qualcosa ce l'ha quasi
 * sempre, e sono capi che a chi ha scelto Blokecore non stonano.
 */
export function paroleDellaFamiglia(nome) {
  const famiglia = FAMIGLIE_STILI.find((f) => f.stili.includes(nome));
  if (!famiglia) return [];
  return [...new Set(famiglia.stili.flatMap((s) => paroleDelloStile(s)))];
}
