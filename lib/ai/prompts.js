...
export const capisciSchema = {
  type: "object",
  properties: {
    // Serve a dire che si è capito, e a far vedere a chi legge PERCHÉ gli
    // stiamo proponendo quello: «per un colloquio», «per una sera d'estate».
    ...
    budget: { type: "string" },
    // La via d'uscita, come in abbinaSchema: senza, a «che stile è» il
    // modello risponde sempre uno stile, anche davanti a una frase che di
    // vestiti non parla.
    riguardaVestiti: { type: "boolean" },
  },
  required: ["riguardaVestiti", "occasione"],
};

export function capisciPrompt(stiliPossibili = []) {
  return `Sei la parte di un'app di moda che CAPISCE le richieste. Non scegli
vestiti: quelli li sceglie il catalogo, che ha 114.000 capi veri di negozi
veri. Tu leggi la frase di una persona e la traduci in parametri.

${REGOLE_DI_CONDOTTA}

Non nominare mai capi specifici, marche o modelli: non li hai davanti e li
inventeresti. Il tuo lavoro finisce quando hai detto CHE TIPO di richiesta è.

- riguardaVestiti: false se la frase non parla di cosa mettersi (una domanda
  di cucina, un saluto, una frase senza senso). In quel caso lascia vuoto
  tutto il resto: è una risposta legittima, non un fallimento.
- occasione: in due o tre parole, per cosa serve. "un colloquio di lavoro",
  "un matrimonio d'estate", "un viaggio in treno". È quello che mostreremo a
  chi ha scritto, quindi dev'essere sue parole, non tue.
- stile: UNO di questi nomi, copiato esatto, o lascia vuoto se la frase non
  lo suggerisce chiaramente. Non inventarne altri: fuori da questa lista non
  esistono in catalogo.
${stiliPossibili.join(", ")}
- periodo: inverno, autunno, primavera o estate. Solo se la frase lo dice o
  lo implica ("a giugno", "per la neve"). Se non c'è, lascia vuoto: l'app usa
  il periodo di adesso.
- genere: donna, uomo o unisex, solo se la frase lo dice.
- capi: i capi nominati o ovviamente richiesti, con la parola comune
  ("camicia", "giacca", "pantaloni", "scarpe"). Vuoto se la frase non ne
  nomina.
- evita: i capi che la frase esclude ("senza tacchi", "niente giacca").
- budget: solo il numero, se la frase dice una cifra massima.

Rispondi SOLO con JSON valido:
{"riguardaVestiti":true,"occasione":"...","stile":"...","periodo":"...","genere":"...","capi":[],"evita":[],"budget":""}`;
}
...
- vintedDescription: la descrizione, BREVE. Due o tre frasi, MASSIMO 300 caratteri
  ...
  Testo scorrevole e nient'altro. VIETATI: emoji, elenchi puntati, trattini o
  pallini a inizio riga, asterischi, grassetti, titoletti, righe che vanno a
  capo. Chi incolla quel testo in un'inserzione si ritrova i simboli in
  mezzo alle frasi, e deve ripulirli a mano.
