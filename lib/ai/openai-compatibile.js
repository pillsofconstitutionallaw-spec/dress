...
export function creaProvider({ nome, url, chiave, modello, timeout = 30000 }) {
  async function chiedi(testo, immagini = []) {
    ...
    const contenuto = immagini.length
  ...
  return {
    ...
    async analyzeColor({ profile = {}, closeup = null, fullbody = null } = {}) {
      const immagini = [closeup, fullbody].filter((u) => typeof u === "string" && u.startsWith("data:"));
      const data = await chiedi(colorPrompt(profile), immagini);
      return {
        season: data.season || "",
...
export const FORNITORI = [
  {
    ...
    modello: () => process.env.MISTRAL_MODEL || "pixtral-12b-2409",
  },
  {
    nome: "groq",
    url: "https://api.groq.com/openai/v1/chat/completions",
    chiave: () => process.env.GROQ_API_KEY,
    modello: () => process.env.GROQ_MODEL || "meta-llama/llama-4-scout-17b-16e-instruct",
  },
  {
    nome: "openrouter",
