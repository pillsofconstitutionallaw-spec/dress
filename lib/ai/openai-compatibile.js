...
import { abbinaPrompt, colorPrompt, vendiPrompt, capisciPrompt } from "./prompts";
...
export function creaProvider({ nome, url, chiave, modello, vista = false, timeout = 30000 }) {
  ...
  // Entrambi i compiti sul capo hanno bisogno della foto e di un modello che
  // sappia guardarla: il controllo è lo stesso, e sta scritto una volta.
  function foto(image) {
  ...
  return {
    ...
    // Capire una frase non ha bisogno di occhi: qui non si manda nessuna
    // foto, solo le parole di chi ha scritto.
    async capisci({ frase = "", stili = [] } = {}) {
      const data = await chiedi(`${capisciPrompt(stili)}\n\nLa richiesta:\n${String(frase).slice(0, 400)}`, []);
      return data;
    },

    async abbina({ image = null } = {}) {
      const data = await chiedi(abbinaPrompt(), foto(image));
      return normalizzaAbbinamento(data);
    },

    async vendi({ image = null } = {}) {
      const data = await chiedi(vendiPrompt(), foto(image));
...
export const FORNITORI = [
  ...
  },
  {
    ...
    modello: () => process.env.GROQ_MODEL || "qwen/qwen3.8-27b",
    // Groq LA VISTA CE L'HA, e questa riga diceva di no.
    //
    // Il commento di prima — «su Groq i modelli con la vista vanno e
    // vengono» — era vero quando è stato scritto e ha smesso di esserlo
    // senza che nessuno lo rileggesse. Nel frattempo llama-3.3-70b-versatile
    // è pure sparito dall'elenco di Groq, quindi questo fornitore non
    // rispondeva più nemmeno per le parole: falliva in silenzio, sempre.
    //
    // qwen3.8-27b guarda le immagini. Provato sulla foto di un paio di
    // Levi's veri: riconosce il capo, legge «Levi's» dall'etichetta, scrive
    // l'annuncio col prezzo e nota i segni d'usura. In 1,2 secondi.
    //
    // Non è qwen3.6, che pure vede: quello risponde ragionando ad alta voce
    // — «<think> The user wants me to…» — e il JSON esce sporco.
    vista: true,
  },
  {
    nome: "openrouter",
