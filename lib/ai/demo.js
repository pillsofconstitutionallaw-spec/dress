...
export const demo = {
  ...
  // Senza AI la frase non si capisce, e fingere di averla capita sarebbe
  // peggio che dirlo: chi ha scritto «per un colloquio» riceverebbe un
  // completo qualunque spacciato per su misura. Si dichiara di non aver
  // capito, e l'app propone quello che proporrebbe comunque.
  async capisci() {
    return { riguardaVestiti: true, occasione: null, capito: false };
  },

  async abbina() {
    return normalizzaAbbinamento({
      isGarment: true,
      objectSeen: "un capo d'abbigliamento",
      title: "Capo in buone condizioni",
      category: "—",
      description:
        "Capo versatile e facile da abbinare, in buone condizioni generali. Vestibilità regolare. (Descrizione dimostrativa: attiva una chiave AI per generarla dalla foto.)",
      matchTips: [
  ...
  async vendi() {
    return normalizzaVendita({
      ...
      description:
        "Capo versatile e facile da abbinare, in buone condizioni generali. Vestibilità regolare. (Descrizione dimostrativa: attiva una chiave AI per generarla dalla foto.)",
      // Nessun prezzo. La descrizione qui sopra dice di sé che è
      // dimostrativa, il prezzo no: usciva «12–20 €» su una foto che nessuno
      // aveva guardato, e questa strada si prende più spesso di quanto
      // sembri — quando i fornitori gratuiti dicono «troppe richieste», che
      // in dieci prove è successo quattro volte. Chi legge un prezzo lo usa.
      priceRange: "",
      // Anche il ripiego dice che la marca manca, invece di tacerla: così chi
      // gira senza chiave AI vede lo stesso riquadro che vedrà con l'AI.
      brand: "",
      vintedTitle: "Capo in buone condizioni",
      vintedDescription:
        "Annuncio dimostrativo: attiva una chiave AI e lo scrivo dalla foto del tuo capo, titolo e descrizione pronti da incollare su Vinted.",
    });
  },
};
