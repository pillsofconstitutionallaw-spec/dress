...
export function normalizzaVendita(data = {}) {
  const g = guardia(data);
  if (!g.riconosciuto) {
    return { ...g, title: "", category: "", description: "", priceRange: "", vintedTitle: "", vintedDescription: "", vintedUrl: null };
  }
  ...
  const vintedTitle = accorcia(senzaTaglia(data.vintedTitle || title), MAX_TITOLO);
  const vintedDescription = frasiEntro(senzaTaglia(data.vintedDescription || description), MAX_DESCRIZIONE);
