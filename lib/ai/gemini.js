...
import { abbinaPrompt, abbinaSchema, capisciPrompt, capisciSchema, colorPrompt, colorSchema, vendiPrompt, vendiSchema } from "./prompts";
...
export const gemini = {
  ...
  // Nessuna foto: qui si legge una frase e basta.
  async capisci({ frase = "", stili = [] } = {}) {
    return generateJSON(
      [{ text: `${capisciPrompt(stili)}\n\nLa richiesta:\n${String(frase).slice(0, 400)}` }],
      capisciSchema,
    );
  },
