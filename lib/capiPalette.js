...
// «mans» stava qui per il possessivo scritto senza apostrofo, come «womans».
// Ma in centomila capi non aggancia un solo capo da uomo: le nove volte che
// compare è sempre Le Mans, la città, e otto cappellini della 24 Ore
// uscivano solo a chi cerca da uomo. «womans» invece un capo ce l'ha, e resta.
const PAROLE_UOMO = /\b(uomo|uomini|maschile|men|mens|men's|man|heren|homme|hommes|hombre|herren)\b/i;
...
// che non escludere.
// Il confine di parola \b di JavaScript conosce solo l'alfabeto inglese:
// dopo la "é" di "Decolleté" pensa che la parola sia già finita, e la voce
// non agganciava. Lo stesso sarebbe successo a "guêpière". Il confine va
// scritto a mano, includendo le lettere accentate — e serve anche dall'altra
// parte, perché "Sergio Tacchini" è una marca di scarpe da ginnastica, non
// un paio di tacchi.
const CONFINE_PRIMA = "(?<![a-zà-ÿ])";
const CONFINE_DOPO = "(?![a-zà-ÿ])";

// Un capo può nominarne un altro senza esserlo. "Camicia con cravatta" è una
// camicia — sei ne avevano una, di Pinko e Silvian Heach, e tre finivano
// schedate da uomo — e i boxer Julipet "fantasia cravatta" sono boxer con
// una stampa. Davanti al nome di un capo queste tre parole lo declassano a
// dettaglio, e un dettaglio non dice per chi è il capo.
//
// Sono tre e non di più perché le altre le smentisce il catalogo: con
// "print" e "detail" sparirebbero centotrenta bikini di Simmi, che si
// chiamano "Floral Print Bikini Bottoms" e "Ruched Detail Bikini Top" —
// lì il capo è proprio quello. Se la parola torna più avanti senza niente
// davanti, come in "Camicia + Cravatta", il capo c'è davvero e vale.
const UN_DETTAGLIO = String.raw`(?<!${CONFINE_PRIMA}(?:con|dettaglio|fantasia)\s["«]?)`;

const capoDi = (voci) => new RegExp(`${CONFINE_PRIMA}${UN_DETTAGLIO}(?:${voci})${CONFINE_DOPO}`, "i");

// Davanti a un altro capo "dress" fa l'aggettivo: i "dress socks" sono i
// calzini da abito e il "dress belt" la cintura da abito, roba da uomo che
// usciva schedata donna. "Dress Blues" e "Dress Bles" sono invece nomi di
// colore di Anerkjendt e Les Deux — un vestito blu si chiama "Dress Blue",
// mai al plurale, e per questo il singolare qui non c'è.
const NON_UN_VESTITO = String.raw`ing|\s*(shirt|socks?|shoes?|boots?|pants?|trousers?|belts?|watch|blues|bles)`;

const CAPI_DA_DONNA = capoDi(
  `gonn[ae]|skirts?|reggisen[oi]|bralette|bikini|gu[eê]pi[eè]re|camicett[ae]|blus[ae]|blouses?|d[eé]collet[eé]s?|tacchi|dress(?!${NON_UN_VESTITO})(es)?`,
);
const CAPI_DA_UOMO = capoDi("cravatt[ae]|papillon|neckties?|tuxedos?");

const chiDice = (donna, uomo) => (donna && uomo ? "unisex" : donna ? "donna" : uomo ? "uomo" : null);

/** Per chi è, letto da quello che il capo dichiara di sé. */
export function perChiE(capo) {
  // Titolo e categoria si tengono separati da una barra: sono due frasi
  // diverse, e incollate con uno spazio ne inventano una terza. La blusa di
  // Rinascimento si chiama "in raso fantasia" e sta nella categoria "Blusa":
  // attaccate danno "fantasia Blusa", che nessuno ha scritto. Oggi non
  // cambia il verdetto di nessuno dei 101.080 capi — il capo è sempre
  // nominato anche altrove — ed è per questo che è una precauzione e non una
  // correzione: la frase inventata prima o poi decide, e non deve.
  const testo = `${capo?.titolo || ""} | ${capo?.categoria || ""}`;
  if (PAROLE_BAMBINO.test(testo)) return "bambino";

  // Prima le parole che dicono per chi è. Quello che dice il capo vale più di
  // quello che dichiara il negozio: il negozio parla di sé in generale, il
  // titolo parla di questo capo.
  const dichiarato = chiDice(PAROLE_DONNA.test(testo), PAROLE_UOMO.test(testo));
  if (dichiarato) return dichiarato;

  // Poi il nome del capo, che è un indizio più debole: dice DI CHE CAPO si
  // tratta, non quanti anni ha chi lo mette. Le gonne le portano anche le
  // bambine, quindi non può smentire un negozio che dichiara «bambino» —
  // ottanta gonne e vestiti da bambina di Sofie Schnoor finivano fra i
  // consigli di una donna adulta proprio così.
  if (capo?.genere === "bambino") return "bambino";

  return chiDice(CAPI_DA_DONNA.test(testo), CAPI_DA_UOMO.test(testo)) || capo?.genere || null;
}
