 ...
 *
 * Il numero era 5, e veniva da una misura fatta quando ogni parola girava su
 * cinque campi di ogni riga, descrizione compresa:
 *
 *    2 parole  1763 ms       6 parole  2875 ms
 *    4 parole  2075 ms       8 parole  TIMEOUT
 *
 * Da allora le parole guardano una colonna sola e indicizzata a trigrammi —
 * vedi sql/ricerca_parole.sql — e quella misura non vale più. Rifatta oggi,
 * stesso database, quattro colori in palette:
 *
 *    1 parola   423 ms       5 parole   908 ms
 *    3 parole   710 ms       8 parole  1180 ms
 *   12 parole  1607 ms
 *
 * Otto sta larghissimo sotto i tre secondi, e cinque lasciava fuori pezzi di
 * famiglia che servono: «bomber» e «giubbino» (169 capi), «loafer» (140),
 * «singlet» (59). Dodici ci starebbe pure, ma non serve a nessuna famiglia
 * di adesso: il tetto è lì per quando qualcuno ne scriverà tre insieme.
 *
 * Le parole scritte da chi cerca restano tutte: si tagliano i sinonimi, e si
 * tagliano dal fondo, dove stanno quelli delle famiglie nominate per ultime.
 *
 ...
 */
const MASSIMO_PAROLE = 8;

/**
 * Via le parole che un'altra già contiene.
 *
 * Nel database il confronto è per pezzo di parola: chi cerca «%giubbott%»
 * trova anche «giubbotto», quindi mandarle tutte e due è un posto sprecato —
 * e i posti sono contati. «Canottiera» ne occupava tre da sola —
 * canottiera, canottier, canott — e lasciava fuori «singlet», che era
 * l'unica a saper prendere quei 59 capi.
 *
 * Si tiene la più corta, che è quella che prende di più.
 */
function senzaDoppioni(parole) {
  const corte = [...parole].sort((a, b) => a.length - b.length);
  const tenute = [];
  for (const p of corte) {
    if (!tenute.some((t) => p.includes(t))) tenute.push(p);
  }
  // si rimette l'ordine di partenza: davanti le parole scritte davvero
  return parole.filter((p) => tenute.includes(p));
}
...
export function paroleEspanse(testo, massimo = MASSIMO_PAROLE) {
  ...
  for (const parola of parole) {
    ...
    for (const voce of famiglia || []) {
      for (const p of parolaCercabile(voce)) fuori.add(p);
    }
  }
  return senzaDoppioni([...fuori]).slice(0, massimo);
}

/**
 * Le parole di una ricerca, raggruppate per come vanno combinate.
 *
 * Torna un gruppo per ogni parola scritta, e dentro ogni gruppo le
 * alternative di quella parola: «camicia bianca» diventa
 * [["camici","shirt","blus","blouse"], ["bianc", ...]].
 *
 * La forma conta. paroleEspanse restituisce tutto appiattito, ed è giusto per
 * chi deve solo allargare la rete; ma chi costruisce una domanda al database
 * deve sapere cosa va in AND e cosa in OR. Mettendo in AND anche i sinonimi
 * si chiede «camici E shirt E blus E bianca», che non esiste in nessun
 * catalogo: zero risultati su ogni ricerca di due parole, e una parola sola
 * che continua a funzionare e fa sembrare tutto a posto.
 *
 * Le parole escono ripulite di tutto ciò che non è una lettera o una cifra.
 * Non è pignoleria: virgole e parentesi hanno un significato nella sintassi
 * di PostgREST, e arrivano da quello che scrive una persona.
 */
export function gruppiPerRicerca(testo) {
  const scritte = String(testo || "")
    .toLowerCase()
    .split(/[^a-z0-9à-ù]+/i)
    .filter((p) => p.length >= 2);

  const gruppi = [];
  for (const parola of scritte) {
    const famiglia = paroleEspanse(parola)
      .map((p) => p.replace(/[^a-z0-9à-ù]/gi, ""))
      .filter(Boolean);
    if (famiglia.length) gruppi.push([...new Set(famiglia)]);
  }
  return gruppi;
}
