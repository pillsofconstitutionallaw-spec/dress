...
/**
 * Un guasto che è nostro, non di chi sta scrivendo.
 *
 * Serve in due posti diversi e deve voler dire la stessa cosa in tutti e due:
 * qui sotto per tradurlo, e nelle route per decidere se un errore si può
 * ingoiare. La differenza conta soprattutto nel recupero password, che
 * risponde sempre uguale per non dire a un estraneo chi è iscritto: quel
 * silenzio è giusto sugli errori che riguardano l'indirizzo, e diventa una
 * promessa falsa quando la richiesta non è proprio partita.
 */
const RETE = /fetch failed|failed to fetch|network|enotfound|econnrefused|econnreset|etimedout|getaddrinfo|socket hang up|operation timed out|und_err/i;

export function guastoDiRete(messaggio) {
  return RETE.test(String(messaggio || ''));
}
...
const MESSAGES = [
  ...
  // La rete che non arriva fino all'archivio. Il client Supabase un guasto
  // così lo consegna come un errore qualunque — «fetch failed» e basta — e
  // da qui usciva intero: non somigliava a nessun messaggio noto e non aveva
  // la faccia da sigla interna, quindi passava. Chi provava a iscriversi
  // vedeva due parole inglesi e nessuna idea di cosa fare.
  //
  // Non serve solo quando un database sparisce: un telefono che perde la
  // linea a metà iscrizione finisce esattamente qui.
  [RETE, 'Non riusciamo a raggiungere il nostro archivio in questo momento. Non è colpa di quello che hai scritto: riprova fra un minuto.'],
...
const DA_NON_MOSTRARE = /jwt|token|claim|postgrest|pgrst|supabase|constraint|violates|null value|syntax error|500|internal|typeerror|referenceerror|undefined|\bat \w+ \(/i;
