...
export const NEGOZI = [
  ...
  { nome: "Marinella", dominio: "emarinella.eu", genere: "uomo", fast: false, fascia: "alto", catalogo: true },
  { nome: "Luigi Fusaro", dominio: "luigifusaro.com", fast: false, fascia: "accessibile", catalogo: true },
  { nome: "Fusaro Uomo", dominio: "fusarouomo.eu", genere: "uomo", fast: false, fascia: "accessibile", catalogo: true },
  { nome: "numb-wear", dominio: "numb-wear.it", fast: false, fascia: "medio", catalogo: true },
  { nome: "Kocca", dominio: "kocca.it", genere: "donna", fast: false, fascia: "medio", catalogo: true },
  { nome: "Rinascimento", dominio: "rinascimento.com", genere: "donna", fast: false, fascia: "medio", catalogo: true },
  { nome: "Vicolo", dominio: "vicolo.com", genere: "donna", fast: false, fascia: "medio", catalogo: true },
  { nome: "Fracomina", dominio: "fracomina.it", genere: "donna", fast: false, fascia: "medio", catalogo: true },
  { nome: "Pinko", dominio: "pinko.com", genere: "donna", fast: false, fascia: "alto", catalogo: true },
...
export function descriviCapo({ capo = "", colore = "", taglia = "", materiale = "", genere = "" } = {}) {
  return [
    capo.trim(),
    // Il genere è la prima parola che toglie di mezzo metà dei risultati, e
    // fino a ieri non arrivava a Google: si impostava "uomo" nel profilo, si
    // premeva "Cerca nei negozi scelti", e la ricerca partiva senza. Da
    // "camicia bianca" a "camicia bianca da uomo" cambia tutto.
    genere === "uomo" ? "da uomo" : genere === "donna" ? "da donna" : "",
    colore.trim(),
    materiale.trim(),
    taglia.trim() && `taglia ${taglia.trim()}`,
  ]
    .filter(Boolean)
    .join(" ")
    .replace(/\s+/g, " ");
...
/**
 * I negozi in cui ha senso cercare, per chi sta cercando.
 *
 * Cinque dei negozi in elenco vendono solo da donna e due solo da uomo:
 * infilarli tutti nella stessa ricerca voleva dire mandare un uomo dentro
 * Kocca, Rinascimento, Vicolo, Fracomina e Pinko, che di roba da uomo non ne
 * hanno neanche una. Non è un filtro in più: è il motivo per cui l'elenco è
 * scelto invece che completo.
 */
// C'è davvero qualcosa da cercare? Il genere da solo non conta: da quando
// entra anche lui nelle parole, "da uomo" bastava a far sembrare valida una
// ricerca in cui non era stato scritto nessun capo.
const qualcosaDaCercare = ({ capo = "", colore = "", materiale = "" } = {}) =>
  Boolean(capo.trim() || colore.trim() || materiale.trim());

export function negoziPerGenere(negozi = [], genere = "") {
  if (!genere) return negozi;
  return negozi.filter((n) => !n.genere || n.genere === genere);
}

// Ricerca su Google Shopping, con fascia di prezzo applicata.
export function urlShopping({ capo, colore, taglia, materiale, genere = "", min, max } = {}) {
  if (!qualcosaDaCercare({ capo, colore, materiale })) return null;
  const q = descriviCapo({ capo, colore, taglia, materiale, genere });
...
export function urlNeiNegozi({ capo, colore, taglia, materiale, genere = "", negozi = [] } = {}) {
  if (!qualcosaDaCercare({ capo, colore, materiale }) || !negozi.length) return null;
  const q = descriviCapo({ capo, colore, taglia, materiale, genere });
