...
/**
 * La palette aggiornata di un'analisi già fatta.
 *
 * Le palette cambiano quando le correggiamo — sono appena passate da cinque
 * colori a dodici. Ma l'analisi resta salvata nel telefono di chi l'ha fatta,
 * e senza questo si porterebbe dietro per sempre la versione vecchia: la
 * correzione non raggiungerebbe proprio le persone che ce l'hanno già.
 *
 * La stagione invece è salvata, e dalla stagione la palette si ricava sempre
 * nella sua forma corrente. Rifare l'analisi non serve.
 */
export function paletteAggiornata(risultato) {
  const corrente = STAGIONI[risultato?.season]?.palette;
  if (corrente?.length) return corrente;
  return risultato?.palette || [];
}
