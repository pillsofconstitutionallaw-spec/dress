...
export function consigliaStili(analisi, profile = {}) {
  ...
  // 3. L'età: una spinta leggera, mai un veto. Il punteggio è basso di
  //    proposito — deve poter essere superato da qualunque altro segnale.
  const eta = etaDa(profile.dataNascita || profile.data_nascita);
  if (eta) {
    const fascia = AFFINITA_ETA.find((f) => eta <= f.fino);
    for (const [i, nome] of (fascia?.spinta || []).entries()) {
      // Nessun motivo nuovo: se uno stile entra solo per l'età, la
      // spiegazione resta quella dei colori. L'età non si nomina.
      // Il peso è vicino a quello del contrasto, non sotto: con 12 punti
      // l'età non spostava nulla e ventidue e sessantasette anni ricevevano
      // gli stessi identici cinque stili. Resta comunque sotto allo stile
      // dichiarato, che vale cento.
      aggiungi(nome, 24 - i * 0.9, PERCHE.vicino);
    }
  }

  // 4. Cinque, non di più: oltre non è un consiglio, è un elenco.
