 ...
 * chiari e i meno colorati della foto: una parete, una camicia bianca — e si
 * calcola di quanto la luce li ha spostati. Poi si riporta indietro tutta
 * l'immagine della stessa quantità.
 *
 * Il riferimento non può essere il soggetto, ed è il difetto che stava qui.
 * Una pelle chiara rosata, sotto una luce azzurrina, perde abbastanza
 * differenza fra i suoi tre canali da passare per neutra — ed essendo più
 * chiara dello sfondo finisce in cima alla fila. Diventava lei il bianco, e
 * la correzione le toglieva esattamente il colore che era lì per misurare:
 * la guancia usciva a 4,9 di «a» e 3,4 di «b», cioè non veniva più
 * riconosciuta nemmeno come pelle. La foto finiva rifiutata, e con la ragione
 * sbagliata — «il viso occupa troppo poco spazio, avvicinati» — quando il
 * viso era grande come prima ed era la luce a non essere stata tolta.
 *
 * Escludendo il volto, sulle stesse foto la correzione torna quella giusta al
 * centesimo: dove ne serviva una di 1,06 · 1,02 · 0,93 ne faceva una di
 * 0,96 · 1,06 · 0,99, cioè storta anche di verso. E dove già funzionava —
 * luce di finestra, lampadina calda — non cambia di un centesimo.
 */

// Dove sta un viso in un ritratto: largo, perché deve coprire anche i capelli
// e il collo. Serve solo a dire da dove NON prendere il bianco, quindi
// abbondare non costa niente — quello che resta fuori è muro, e il muro è
// esattamente quello che cerchiamo.
const NEL_VOLTO = (p) => p.x > 0.18 && p.x < 0.82 && p.y > 0.12 && p.y < 0.78;

export function correggiLuce(rgb) {
  // Candidati neutri: chiari, con i tre canali già vicini fra loro, e fuori
  // dal viso. Un pixel senza posizione — le prove che chiamano questa
  // funzione da sola — non è dentro nessun volto.
  const candidati = rgb.filter((p) => {
    if (p.x != null && p.y != null && NEL_VOLTO(p)) return false;
    const { r, g, b } = p;
...
function valutaCondizioni({ pixel, luce, pelleCandidata, centrali }) {
  ...
  // La luce prima dell'inquadratura, ed è l'ordine giusto per chi legge: se
  // la luce non si può togliere, la pelle non viene riconosciuta come pelle e
  // scatta ANCHE «il viso occupa troppo poco spazio». Quello è il sintomo;
  // la causa è la luce, ed è l'unica delle due su cui si può fare qualcosa.
  // Detto per primo, si rifà la foto vicino a una finestra invece di rifarla
  // più da vicino sotto la stessa lampadina.
