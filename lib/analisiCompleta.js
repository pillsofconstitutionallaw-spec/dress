...
function cosaManca(nostra, { closeup, fullbody, profile }) {
  const mancanze = [];
  // Chi ha corretto a mano il sottotono ha già deciso: elencargli i motivi
  // per cui la misura era debole sarebbe una lezione su una domanda chiusa.
  if (nostra.misura?.corretta) return mancanze;
 ...
 */
export async function eseguiAnalisi({
  profile = {}, closeup = null, fullbody = null, testRisposte = null, correzione = null,
} = {}) {
  const nostra = await analizzaColori({ profile, closeup, fullbody, testRisposte, correzione });
...
export async function salvaAnalisi({ risultato, profile = {}, budget = "", testRisposte = null, correzione = null, stileScelto } = {}) {
  try {
    await apiFetch("/api/profile/save", {
      ...
      body: {
        ...
        dati: {
          ...
          // La correzione fatta a mano viaggia con l'analisi: è il pezzo che
          // scavalca tutto il resto, e riaprire l'app senza di lei vorrebbe
          // dire tornare alla stagione che la persona aveva già rifiutato.
