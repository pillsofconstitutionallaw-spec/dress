 ...
 */
/**
 * Da dove torna chi arriva da una mail, e dove va portato.
 *
 * Il link «ho dimenticato la password» non atterra sulla pagina che
 * reimposta la password: Supabase rimanda all'indirizzo del sito e basta.
 * Provato chiedendogli tre indirizzi diversi — compreso quello esatto della
 * pagina giusta — e tutte e tre le volte il link rimandava alla home. È
 * l'elenco degli indirizzi permessi del progetto, e si sistema di là.
 *
 * Ma l'app non può dipendere da un elenco che non controlla, e il pezzo che
 * tocca a lei era peggio della configurazione: la home non guardava il
 * frammento. Il client Supabase la sessione di recupero la apre da solo, e
 ...
 * dopo era di nuovo fuori, con lo stesso link che la riportava nello stesso
 * punto.
 *
 * Il frammento però dice sempre da dove si viene. Basta leggerlo.
 */
const DOVE = { recovery: "/auth/reimposta", signup: "/auth/confirmed", email_change: "/auth/confirmed" };

export function doveMandare(pezzo) {
  const testo = String(pezzo ?? "").replace(/^[#?]/, "");
  if (!testo) return null;
  const tipo = new URLSearchParams(testo).get("type");
  return DOVE[tipo] || null;
}
